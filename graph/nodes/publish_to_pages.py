...
def publish_to_pages(state: State) -> dict:
    ...
    content_path = f"content/ai-daily/{today}.html"
    ...
    _create_or_update_file(
        repo, headers, content_path, html_content, f"feat: add ai-daily {today}"
    )

    # 2. 更新 index.json
    manifest_file = _get_file(repo, headers, manifest_path)
    manifest_data = json.loads(manifest_file["content"])
    if today not in manifest_data.get("ai-daily", []):
        manifest_data.setdefault("ai-daily", []).insert(0, today)
        _create_or_update_file(
            repo,
            headers,
            manifest_path,
    ...
    logger.info("已发布 ai-daily %s 到 GitHub Pages", today)
    return {}
